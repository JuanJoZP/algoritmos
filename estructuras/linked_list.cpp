...
class Point {
 ...
 public:
  ...
  Point ofuscacion() {
    return Point(x + 2 * (rand() % rmax) - rmax,
                 y + 2 * (rand() % rmax) - rmax);
  }

  double dist() { return dist(0, 0); }

  double dist(int xc, int yc) { return sqrt(pow(x - xc, 2) + pow(y - yc, 2)); }

  double dist(Point p) { return dist(p.x, p.y); }
...
class Nodo {
 ...
 public:
  ...
  Point* getDato() { return dato; }

  void setDato(Point* d) { dato = d; }

  Nodo* getNext() { return pointer; }

  void setNext(Nodo* p) { pointer = p; }

  void setNext() { pointer = NULL; }
};
...
class Lista {
 ...
 public:
  ...
  void setPtr(Nodo* ptrr) { ptr = ptrr; }
