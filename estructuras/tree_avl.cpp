...
class Node {
 ...
 public:
  ...
  Node* getIzq() { return izq; }

  Node* getDer() { return der; }

  void setIzq(Node* iz) { izq = iz; }

  void setDer(Node* de) { der = de; }

  bool hasIzq() { return izq != NULL; }

  bool hasDer() { return der != NULL; }
...
class Tree {
 ...
 public:
  ...
  bool isIzqHeavy() { return isIzqHeavy(root); }

  bool isDerHeavy(Node* r) {
    return getHeight(r->getIzq()) - getHeight(r->getDer()) < -1;
  }

  bool isDerHeavy() { return isDerHeavy(root); }
  ...
  Node* parent(int ch) { return parent(ch, root); }
